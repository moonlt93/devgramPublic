package com.project.devgram.chat.handler;
...
public class StompHandler implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if (StompCommand.CONNECT == accessor.getCommand()) { // websocket 연결요청
            ...
            chatRoomRepository.setUsernameInfo(sessionId,jwtToken);
            ...
            String name= tokenService.getUsername(chatRoomRepository.getTokenInfo(sessionId));
            ...
            log.info("count: {}",chatRoomRepository.getUserCount(roomId));

            //채팀방 인원이 아무도 없을때 방은 사라진다.
            if(chatRoomRepository.getUserCount(roomId) < 1) {
                assert sessionId != null;
                if (!sessionId.isEmpty()) {
                    chatRoomRepository.deleteById(sessionId, roomId);
                }
            }
